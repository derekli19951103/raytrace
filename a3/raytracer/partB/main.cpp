...
int dof(int width,int height) {
    ...
    Camera camera1(Point3D(0, 0, 1), Vector3D(0, 0, -1), Vector3D(0, 1, 0), 60.0);
    Image image1(width, height);
    raytracer.render_dof(camera1, scene, light_list, image1); //render 3D scene to image
    image1.flushPixelBuffer("DOF1.bmp"); //save rendered image to file
    
    // Render it from a different point of view.
    Camera camera2(Point3D(4, 2, 1), Vector3D(-4, -2, -6), Vector3D(0, 1, 0), 60.0);
    Image image2(width, height);
    raytracer.render_dof(camera2, scene, light_list, image2);
    image2.flushPixelBuffer("DOF2.bmp");
...
int full(int width,int height) {
    ...
    sphere_nog->translate(Vector3D(0, 0, -5));
    sphere_nog->rotate('z', 45);
    sphere_nog->scale(Point3D(0, 0, 0), factor1);
    
    double factor2[3] = { 6.0, 6.0, 6.0 };
    plane->translate(Vector3D(-3, -1, -7));
    plane->rotate('x', -60);
    plane->scale(Point3D(0, 0, 0), factor2);
    
    double factor3[3] = { 2.0, 1.0, 1.0 };
    cube->translate(Vector3D(4, 0, -5));
    cube->scale(Point3D(0, 0, 0), factor3);
    
    double factor4[3] = { 1.0, 2.0, 1.0 };
    sphere->translate(Vector3D(-4, 0, -4));
    sphere->scale(Point3D(0, 0, 0), factor4);
